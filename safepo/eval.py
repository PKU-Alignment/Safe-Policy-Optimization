...
def get_defaults_kwargs_yaml(path):
    path = (
        ...
        if path[0] == "/"
        else os.path.join(*path.split("/")[:-1])
    )
...
if __name__ == "__main__":
    ...
    *./mp4 files will be saved in your model directory.
    ...
    parser.add_argument("--seed", default=0, type=int, help="The seed for testing.")
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    parser.add_argument("--width", type=int, default=512, help="The movies width.")
    parser.add_argument("--height", type=int, default=512, help="The movies height.")

    args, unparsed_args = parser.parse_known_args()
    default_log_dir = os.path.join(args.path, "./evals")

    assert (
        args.model_name is not None or args.all
    ), "Please specify the way you want to eval models."
    ...
    if args.all:
        for item in os.scandir(path):
            if item.is_file() and item.name.split(".")[-1] == "pt":
                model.ac.pi.load_state_dict(
                    torch.load(
                        os.path.join(path, item.name), map_location=torch.device("cpu")
                    )
                )
        ...
        model.ac.pi.load_state_dict(
            torch.load(
                os.path.join(path, args.model_name + ".pt"),
        ...
        for i in range(args.eval_ep):
            ...
            while not d:
                a, v, cv, logp = model.ac.step(torch.as_tensor(o, dtype=torch.float32))
                o, r, d, env_info = env.step(a)
