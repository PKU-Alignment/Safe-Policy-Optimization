...
def retrieve_cfg(args, use_rlg_config=False):
    ...
    if args.task == "ShadowHandOver":
        return os.path.join(args.logdir, "shadow_hand_over/{}/{}".format(args.algo, args.algo)), "marl_marl_cfg/{}/config.yaml".format(args.algo) , "marl_cfg/shadow_hand_over.yaml"
    elif args.task == "ShadowHandCatchOverarm":
        return os.path.join(args.logdir, "shadow_hand_catch_overarm/{}/{}".format(args.algo, args.algo)), "marl_cfg/{}/config.yaml".format(args.algo), "marl_cfg/shadow_hand_catch_overarm.yaml"
        ...
        return os.path.join(args.logdir, "shadow_hand_two_catch_underarm/{}/{}".format(args.algo, args.algo)), "marl_cfg/{}/config.yaml".format(args.algo), "marl_cfg/shadow_hand_two_catch_underarm.yaml"
    elif args.task == "ShadowHandCatchAbreast":
        return os.path.join(args.logdir, "shadow_hand_catch_abreast/{}/{}".format(args.algo, args.algo)), "marl_cfg/{}/config.yaml".format(args.algo), "marl_cfg/shadow_hand_catch_abreast.yaml"
...
def load_cfg(args, use_rlg_config=False):
    ...
    cfg["name"] = args.task
    agent_conf = cfg["env"]["agent_conf"]
    scenario = cfg["env"]["scenario"]


    cfg["headless"] = args.headless
    cfg_train["env_name"] = "Safety"+agent_conf+scenario.split("-")[0]+"Velocity-v0"
    cfg_train["n_rollout_threads"] = args.n_rollout_threads
    cfg_train["agent_conf"] = agent_conf
    cfg_train["scenario"] = scenario
...
def get_args(benchmark=False, use_rlg_config=False, **kwargs):
    ...
    if args.algo not in ["happo", "mappo","ippo","ppo","sac","td3","ddpg","trpo","macpo", "mappolag"]:
        warn_algorithm_name()
