...
class FOCOPS(PG, Lagrangian):
    ...
    def compute_loss_pi(self, data: dict):
        ...
        kl_new_old = torch.distributions.kl.kl_divergence(dist, self.p_dist).sum(
            -1, keepdim=True
        )
        loss_pi = (
            kl_new_old
            - (1 / self.lam)
            * ratio
            * (data["adv"] - self.lagrangian_multiplier * data["cost_adv"])
    ...
    def update_policy_net(self, data) -> None:
        ...
        for i in range(self.train_pi_iterations):
            ...
            torch_kl = (
                torch.distributions.kl.kl_divergence(q_dist, self.p_dist).mean().item()
            )
