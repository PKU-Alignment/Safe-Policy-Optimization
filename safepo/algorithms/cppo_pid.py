...
def parse_args():
    ...
    parser.add_argument("--pid-kp", type=float, default=0.1,
        help="the Kp of PID controller")
    parser.add_argument("--pid-ki", type=float, default=0.01,
        help="the Ki of PID controller")
    parser.add_argument("--pid-kd", type=float, default=0.0,
        help="the Kd of PID controller")
    parser.add_argument("--pid-d-delay", type=int, default=10,
        help="the delay of derivative term")
    parser.add_argument("--pid-delta-p-ema-alpha", type=float, default=0.95,
        help="the exponential moving average alpha of the proportional term of the PID controller.")
    parser.add_argument("--pid-delta-d-ema-alpha", type=float, default=0.95,
        help="the exponential moving average alpha of the derivative term of the PID controller.")
    parser.add_argument("--sum-norm", type=lambda x: bool(strtobool(x)), default=True,
        help="toggles to normalize the sum of the cost")
    parser.add_argument("--diff-norm", type=lambda x: bool(strtobool(x)), default=True,
        help="toggles to normalize the derivate of the cost")
    parser.add_argument("--penalty-max", type=float, default=100.0,
        help="the max penalty coefficient")
...
def main(args):
    ...
    # set up the logger
    dict_args = vars(args)
    exp_name = "-".join([args.env_id, "cppopid"])
    logger = EpochLogger(
...
if __name__ == "__main__":
    ...
    args.log_dir = os.path.join(args.log_dir, args.env_id, "cppo_pid", relpath)
