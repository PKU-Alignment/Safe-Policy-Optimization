...
def retrieve_cfg(args, use_rlg_config=False):
    ...
    if args.task == "ShadowHandOver":
        ...
        return os.path.join(args.logdir, "ShadowHandOver_Safe_joint/{}/{}".format(args.algo, args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/ShadowHandOver_Safe_joint.yaml"
    elif args.task == "ShadowHandOver_Safe_finger":
        return os.path.join(args.logdir, "ShadowHandOver_Safe_finger/{}/{}".format(args.algo, args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/ShadowHandOver_Safe_finger.yaml"
    elif args.task == "ShadowHandCatchOver2Underarm_Safe_finger":
        return os.path.join(args.logdir, "ShadowHandCatchOver2underarm_Safe_finger/{}/{}".format(args.algo, args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/ShadowHandCatchOver2underarm_Safe_finger.yaml"
    elif args.task == "ShadowHandCatchOver2Underarm_Safe_joint":
        return os.path.join(args.logdir, "ShadowHandCatchOver2underarm_Safe_joint/{}/{}".format(args.algo, args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/ShadowHandCatchOver2underarm_Safe_joint.yaml"
...
def get_args(benchmark=False, use_rlg_config=False):
    ...
    if args.algo not in ["maddpg", "happo", "mappo", "hatrpo","ippo","ppo","sac","td3","ddpg","trpo","ppol","focops","p3o", "pcpo", "cpo", "trpol"]:
        warn_algorithm_name()
