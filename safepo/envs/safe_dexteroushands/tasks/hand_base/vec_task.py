...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        return torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.cost_buf.to(self.rl_device),self.task.reset_buf.to(self.rl_device), self.task.extras
