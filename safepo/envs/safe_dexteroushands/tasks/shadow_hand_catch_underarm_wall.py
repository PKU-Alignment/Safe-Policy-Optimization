...
class ShadowHandCatchUnderarmWall(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        wall_texture_files = "assets/textures/texture_stone_stone_texture_0.jpg"
        wall_texture_handle = self.gym.create_texture_from_file(self.sim, wall_texture_files)
        ...
        shadow_another_hand_start_pose.p = gymapi.Vec3(0, -1.05, 0.5)
        ...
        # create wall asset
        wall_dims = gymapi.Vec3(1, 0.03, 0.4)
        asset_options = gymapi.AssetOptions()
        ...
        wall_pose1 = gymapi.Transform()
        wall_pose1.p = gymapi.Vec3(0, -0.525, 22.95)
        wall_pose1.r = gymapi.Quat().from_euler_zyx(-0., 0, 0)
        wall_pose2 = gymapi.Transform()
        wall_pose2.p = gymapi.Vec3(0, -0.525, 22.35)
        wall_pose2.r = gymapi.Quat().from_euler_zyx(-0., 0, 0)
        wall_pose3 = gymapi.Transform()
        wall_pose3.p = gymapi.Vec3(0.3, -0.525, 22.65)
        wall_pose3.r = gymapi.Quat().from_euler_zyx(-0., 1.57, 0)
        wall_pose4 = gymapi.Transform()
        wall_pose4.p = gymapi.Vec3(-0.3, -0.525, 22.65)
        wall_pose4.r = gymapi.Quat().from_euler_zyx(-0., 1.57, 0)

        # compute aggregate size
        max_agg_bodies = self.num_shadow_hand_bodies * 2 + 2 + 4
        max_agg_shapes = self.num_shadow_hand_shapes * 2 + 2 + 4
        ...
        for i in range(self.num_envs):
            ...
            # add wall 
            wall_handle = self.gym.create_actor(env_ptr, wall_asset, wall_pose1, "wall1", i + self.num_envs, -1, 0)
            self.gym.set_rigid_body_color(env_ptr, wall_handle, 0, gymapi.MESH_VISUAL,
                                    gymapi.Vec3(1, 228/255, 196/255))            
            wall_idx = self.gym.get_actor_index(env_ptr, wall_handle, gymapi.DOMAIN_SIM)
            self.wall_indices.append(wall_idx)
            wall_handle = self.gym.create_actor(env_ptr, wall_asset, wall_pose2, "wall2", i + self.num_envs, -1, 0)
            self.gym.set_rigid_body_color(env_ptr, wall_handle, 0, gymapi.MESH_VISUAL,
                                    gymapi.Vec3(1, 228/255, 196/255))      
            wall_idx = self.gym.get_actor_index(env_ptr, wall_handle, gymapi.DOMAIN_SIM)
            self.wall_indices.append(wall_idx)
            wall_handle = self.gym.create_actor(env_ptr, wall_asset, wall_pose3, "wall3", i + self.num_envs, -1, 0)
            self.gym.set_rigid_body_color(env_ptr, wall_handle, 0, gymapi.MESH_VISUAL,
                                    gymapi.Vec3(1, 228/255, 196/255))                  
            wall_idx = self.gym.get_actor_index(env_ptr, wall_handle, gymapi.DOMAIN_SIM)
            self.wall_indices.append(wall_idx)
            wall_handle = self.gym.create_actor(env_ptr, wall_asset, wall_pose4, "wall4", i + self.num_envs, -1, 0)
            self.gym.set_rigid_body_color(env_ptr, wall_handle, 0, gymapi.MESH_VISUAL,
                                    gymapi.Vec3(1, 228/255, 196/255))                  
            wall_idx = self.gym.get_actor_index(env_ptr, wall_handle, gymapi.DOMAIN_SIM)
            self.wall_indices.append(wall_idx)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.consecutive_successes[:], reward_detail = compute_hand_reward(
    ...
    def compute_cost(self, actions):
        ...
        y_in_wall = torch.abs(self.object_pos[:, 1] + 0.525) < 0.015
        collision = (torch.abs(self.object_pos[:, 0])>0.25) | (self.object_pos[:, 1]>0.70) | (self.object_pos[:, 1]<0.60)
        self.cost_buf = torch.where(y_in_wall & collision, torch.ones_like(self.cost_buf), self.cost_buf)
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        self.goal_states[env_ids, 1] -= 0.27
    ...
    def post_physics_step(self):
        ...
        self.compute_cost(self.actions)
...
def compute_hand_reward(
    ...
    dist_reward = 5*torch.exp(-0.2*(dist_rew * dist_reward_scale + rot_dist))
    ...
    success_reward = torch.where(goal_resets == 1, reach_goal_bonus, 0.)

    # Fall penalty: distance to the goal is larger than a threashold
    fall_reward = torch.where(object_pos[:, 2] <= 0.1, fall_penalty, 0.)
    ...
    reward = dist_reward + success_reward + fall_reward
    # Apply penalty for not reaching the goal
    if max_consecutive_successes > 0:
        penalty_reward = torch.where(progress_buf >= max_episode_length, 0.5 * fall_penalty, 0.)
        reward += penalty_reward
        print("penalty_reward", penalty_reward.mean().item())
        reward_detail = {"total reward": reward.mean(), "dist reward": dist_reward.mean(), "success_reward": success_reward.mean(), "fall_reward": fall_reward.mean(), "penalty_reward": penalty_reward.mean()}
    else:
        reward_detail = {"total reward": reward.mean(), "dist reward": dist_reward.mean(), "success_reward": success_reward.mean(), "fall_reward": fall_reward.mean()}
    ...
    return reward, resets, goal_resets, progress_buf, successes, cons_successes, reward_detail
