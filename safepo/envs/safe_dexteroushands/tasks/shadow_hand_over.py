...
class ShadowHandOver(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        shadow_another_hand_start_pose.r = gymapi.Quat().from_euler_zyx(0, 0, 3.1415926)
    ...
    def compute_cost(self):
        ...
        self.cost_buf = torch.where(actions[:, 4] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 4+24] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 4+24] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        # self.cost_buf = torch.where(actions[:, 17] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        # self.cost_buf = torch.where(actions[:, 21] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        # self.cost_buf = torch.where(actions[:, 20] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
