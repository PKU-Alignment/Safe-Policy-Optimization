...
class ShadowHandOverWallDown(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        wall_pose.p = gymapi.Vec3(0, -0.525, self.wall_height/2.+20)
    ...
    def compute_cost(self, actions):
        ...
        self.cost_buf = torch.where(torch.abs(self.object_pos[:, 1] - (-0.525)) < 0.015,
                            torch.where(self.object_pos[:, 2]< self.wall_height, torch.ones_like(self.cost_buf), self.cost_buf), self.cost_buf)
