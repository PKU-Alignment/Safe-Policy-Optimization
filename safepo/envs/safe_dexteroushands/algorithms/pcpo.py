    ...
    def update(self, avg_cost):
        ...
        for epoch in range(self.num_learning_epochs):
            for indices in batch:
                ...
                r = torch.matmul(approx_g, H_inv_a) # g^T * H^-1 * a
                approx_a = Fvp(H_inv_a)
                s = torch.matmul(approx_a, H_inv_a) # a^T * H^-1 * a
                x = torch.sqrt(2 * self.max_kl / (q+EPS)) * H_inv_g - torch.clamp_min(
                    (torch.sqrt(2 * self.max_kl/q) * s + avg_cost) / s, torch.tensor(0.0)
                ) * H_inv_a
                ...
                def set_and_eval(step):
                    ...
                    if kl_value <= self.max_kl and pi_l_new >= surrogate_loss and improve_cost <= max(-c, 0.0):
                        return True
                    else:
                        return False
