            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    if self.apply_reset:
                        ...
                        current_states = self.vec_env.get_state()
                    # Compute the action
                    actions, actions_log_prob, mu, sigma = self.actor.act(current_obs, current_states)
                    ...
                    self.storage.add_transitions(current_obs, current_states, actions, rews, costs, dones, values, cost_values, actions_log_prob, mu, sigma)
                    current_obs.copy_(next_obs)
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                actions_log_prob_batch, entropy_batch, mu_batch, sigma_batch = self.actor.evaluate(obs_batch,states_batch,actions_batch)
                ...
                # cost function loss
                if self.use_clipped_value_loss:
                    cost_value_clipped = cost_target_values_batch + (cost_value_batch - cost_target_values_batch).clamp(-self.clip_param,self.clip_param)
                ...
                loss = (kl_mean - (1 / self.focops_lam) * ratio * (torch.squeeze(advantages_batch) - self.nu * torch.squeeze(cadvantages_batch))) \
