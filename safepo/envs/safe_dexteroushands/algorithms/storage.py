...
class RolloutStorage:
    ...
    def add_transitions(self, observations, states, actions, rewards, costs, dones, values, cost_values, actions_log_prob, mu, sigma):
    ...
    def compute_costs(self, last_cost_values, gamma, lam):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            next_is_not_terminal = 1.0 - self.dones[step].float()
            delta = self.costs[step] + next_is_not_terminal * gamma * next_cvalues - self.cost_values[step]
        ...
        self.cadvantages = (self.cadvantages - self.cadvantages.mean()) / (self.cadvantages.std() + 1e-8)
