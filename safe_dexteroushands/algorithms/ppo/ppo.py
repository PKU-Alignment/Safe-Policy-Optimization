...
class PPO:
    ...
    def __init__(self,
        ...
        self.device = device
        # self.device = torch.device('cuda', int(device)) if torch.cuda.is_available() else torch.device('cpu')
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    self.storage.add_transitions(current_obs, current_states, actions, rews, costs, dones, values, actions_log_prob, mu, sigma)
                    current_obs.copy_(next_obs)
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if locs['ep_infos']:
            for key in locs['ep_infos'][0]:
                ...
                self.writer.add_scalar('dEpisode/' + key, value, locs['it'])
        ...
        for epoch in range(self.num_learning_epochs):
            for indices in batch:
                ...
                if self.desired_kl != None and self.schedule == 'adaptive':
                    ...
                    assert kl_mean.item() >= 0, "kl_mean is negative!!!"
