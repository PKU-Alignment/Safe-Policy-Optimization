...
class ShadowHandCatchOver2Underarm_Safe_finger(BaseTask):
    ...
    def compute_cost(self):
        ...
        self.cost_buf = torch.where(actions[:, 2] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 2] > -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 3] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 3] > -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
