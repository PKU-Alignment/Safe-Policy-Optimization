...
class ShadowHandCatchUnderarm(BaseTask):
    ...
    def compute_cost(self):
        ...
        # self.cost_buf = torch.where(actions[:, 21] < -0.8, torch.ones_like(self.cost_buf), self.cost_buf)
        # self.cost_buf = torch.where(actions[:, 21] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        # self.cost_buf = torch.where(actions[:, 20] < -0.8, torch.ones_like(self.cost_buf), self.cost_buf)
