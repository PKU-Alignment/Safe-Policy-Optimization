...
class ShadowHandDoorCloseInward(BaseTask):
    ...
    def compute_cost(self):
        ...
        self.cost_buf = torch.where(actions[:, 10] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 10] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf)

        self.cost_buf = torch.where(actions[:, 36] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf)
        self.cost_buf = torch.where(actions[:, 36] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf)
